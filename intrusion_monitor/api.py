...
rc = CachedSession(expire_after=604800)
...
def api_call(ip, fields_id=66846719):
    ...
    try:
        ...
        req = rc.get(req_str)
        logging.debug(f'\t> Got HTTP code {req.status_code}. Request from cache: {req.from_cache}')
    except:
        logging.error(f'Something occurred while getting API connection...', exc_info=True)
        return None
